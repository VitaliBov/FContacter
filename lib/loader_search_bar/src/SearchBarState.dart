...
import 'package:f_contacter/loader_search_bar/src/QuerySetLoader.dart';
import 'package:f_contacter/loader_search_bar/src/SearchBar.dart';
import 'package:f_contacter/loader_search_bar/src/SearchBarBuilder.dart';
import 'package:f_contacter/loader_search_bar/src/StateHolder.dart';

abstract class SearchBarState extends State<SearchBar> {
  SearchBarState();

  factory SearchBarState.create(SearchBar searchBar) => searchBar.iconified ? IconifiedBarState() : MergedBarState();
  ...
  VoidCallback get clearQueryCallback => () {
    if (widget.controller?.onClearQuery == null) {
      onClearQuery();
    } else {
      widget.controller?.onClearQuery();
      onClearQuery();
    }
  };

  VoidCallback get cancelSearchCallback => () {
    if (widget.controller?.onCancelSearch == null) {
      onCancelSearch();
    } else {
      widget.controller?.onCancelSearch();
      onCancelSearch();
    }
  };
  ...
  void onClearQuery() => queryNotEmpty ? _clearQueryField() : cancelSearchCallback();
  ...
  void _requestSearchFocus() => FocusScope.of(context).requestFocus(searchFocusNode);
